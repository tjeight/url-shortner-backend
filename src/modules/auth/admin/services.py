...
async def admin_signup_post(
    db: AsyncSession, payload: AdminRegisterPostRequestModel
) -> JSONResponse:
    try:
        ...
        return JSONResponse(
            ...
            content={"success": True, "message": "Admin Registered Successfully"},
        )
...
async def admin_login_post(
    db: AsyncSession, payload: AdminLoginPostRequestModel, request: Request
) -> JSONResponse:
    try:
        start_time = perf_counter()
        # Fetch admin from users table where role is admin
        result = await db.execute(
            select(Users).where(
                Users.user_email == payload.user_email,
        ...
        if not admin or not verify_hashed_password(
        ...
        start_time = perf_counter()
        # Create session in the shared user_sessions table
...
async def refresh_access_token_get(auth: AdminAuth):
    try:
...
async def forgot_password_email_verification_post(
    db: AsyncSession, payload: AdminForgotPasswordPostRequestModel
):
    try:
        # Check email exists in the users table with admin role
        email_fetch_query = await db.execute(
            select(Users).where(
                Users.user_email == payload.user_email,
                Users.role == UserRole.ADMIN.value,
        ...
        admin_name = f"{email_result.first_name} {email_result.last_name}"
        ...
        is_email_sent = await send_email(
            ...
            html_content=build_otp_email(
                otp=otp, user_name=admin_name, app_name=app_name
            ),
        )
        ...
        # Set the otp in the redis with admin prefix to avoid collision with user OTPs
        await redis_client.set(
            f"admin_forgot_password_otp:{payload.user_email}", otp, ex=300
        )
...
# Function to verify the forgot password otp sent to the admin
async def forgot_password_otp_verification_post(
    db: AsyncSession, payload: AdminForgotPasswordVerifyOTPPostRequestModel
):
    try:
        # Get the otp from the redis using admin prefix
        stored_otp = await redis_client.get(
            f"admin_forgot_password_otp:{payload.user_email}"
        )
        ...
        await redis_client.delete(f"admin_forgot_password_otp:{payload.user_email}")

        # Fetch the admin details from the users table
        email_fetch_query = await db.execute(
            select(Users).where(
                Users.user_email == payload.user_email,
                Users.role == UserRole.ADMIN.value,
        ...
        admin = email_fetch_query.scalar_one()
...
async def reset_password_post(
    db: AsyncSession, payload: AdminResetPasswordPostRequestModel
):
    try:
        ...
        # Fetch the admin from the users table using user_id from reset token
        email_fetch_query = await db.execute(
            select(Users).where(
                Users.user_id == user_reset_auth.user_id,
                Users.role == UserRole.ADMIN.value,
            )
        )
        admin = email_fetch_query.scalar_one_or_none()
...
# Function to handle the admin logout and delete the session from the database
async def admin_logout_post(db: AsyncSession, auth: AdminAuth):
    try:
        # Fetch the session from the shared user_sessions table
        ...
        if not session:
            return JSONResponse(
                status_code=status.HTTP_401_UNAUTHORIZED,
                content={"success": False, "message": "Invalid Admin Session"},
            )
        ...
        await db.rollback()
        logger.error(f"Failed to logout the admin: {e}", exc_info=True)
        return JSONResponse(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            content={"success": False, "message": "Internal Server Error"},
        )
